...
Define_Module(UDPBasicFlood);

simsignal_t UDPBasicFlood::sentPkSignal = SIMSIGNAL_NULL;
simsignal_t UDPBasicFlood::rcvdPkSignal = SIMSIGNAL_NULL;

UDPBasicFlood::UDPBasicFlood()
{
    selfMsg = NULL;
}
...
IPvXAddress UDPBasicFlood::chooseDestAddr()
{
    int k = intrand(destAddresses.size());
    return destAddresses[k];
}

void UDPBasicFlood::sendPacket()
{
    char msgName[32];
    sprintf(msgName, "UDPBasicAppData-%d-%d", mod_id, numSent);
...
void UDPBasicFlood::processStart()
{
...
void UDPBasicFlood::processSend()
{
    sendPacket();
...
void UDPBasicFlood::processStop()
{
    socket.close();
}

void UDPBasicFlood::handleMessageWhenUp(cMessage *msg)
{
...
bool UDPBasicFlood::stopApp(IDoneCallback *doneCallback)
{
    if (selfMsg)
        cancelEvent(selfMsg);
...
bool UDPBasicFlood::crashApp(IDoneCallback *doneCallback)
{
    if (selfMsg)
        cancelEvent(selfMsg);
    return true;
