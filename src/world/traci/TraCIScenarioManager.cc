...
void TraCIScenarioManager::init_traci() {
	{
		...
		if ((apiVersion == 2) || (apiVersion == 3) || (apiVersion == 5) || (apiVersion == 6)) {
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf) {
    ...
    for (uint8_t j = 0; j < variableNumber_resp; ++j) {
        ...
        if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS) {
            ...
            //printf("[FR] active %d | teleport %d | count %d | deleted %d\n", activeVehicleCount, teleportedVehiclesCount, count, carsAlreadyDeleted);
            ...
            activeVehicleCount -= (count - carsAlreadyDeleted);
            //printf("[IT] active %d | teleport %d | count %d | deleted %d\n", activeVehicleCount, teleportedVehiclesCount, count, carsAlreadyDeleted);
            ...
            teleportedVehiclesCount -= count;
            activeVehicleCount += count;
            //printf("[FT] active %d | teleport %d | count %d\n", activeVehicleCount, teleportedVehiclesCount, count);
       ...
       /* } else if (variable1_resp == VAR_MIN_EXPECTED_VEHICLES) {
            ...
            //printf("[LV] active %d | teleport %d | count %d\n", activeVehicleCount, teleportedVehiclesCount, count);
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf) {
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j) {
		...
		if (isokay != RTYPE_OK) {
	        ...
	        //printf("active %d | teleport %d | count %d\n", activeVehicleCount, teleportedVehiclesCount, count);
